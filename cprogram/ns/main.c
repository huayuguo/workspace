...
void noise_suppression(char *in_file, char *out_file) {
    ...
    if (inBuffer != nullptr) {
		...
		printf("noise_suppression: sampleRate = %d, inSampleCount = %d, channels = %d\n", sampleRate, (int)inSampleCount, channels);
        nsProcess(inBuffer, sampleRate, inSampleCount, channels, kVeryHigh);
