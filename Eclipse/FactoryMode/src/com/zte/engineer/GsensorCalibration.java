...
public class GsensorCalibration extends ZteActivity {
	...
	boolean isCalibrationFinish = false;
    int temp = 0;
    int temp1 = 2;
	int count = 0;
	int number = 0;
    ...
    private void initUi() {
		...
		((Button) findViewById(R.id.btnPass)).setVisibility(View.GONE);
		((Button) findViewById(R.id.btnFail)).setVisibility(View.GONE);
	...
	private Handler mHandler1 = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (result == Util.RET_SUCCESS) {
				...
				((Button) findViewById(R.id.gyrosensor)).setText(R.string.calibrate_finish);
				isCalibrationFinish = true;
       ...
       // List<Sensor> sensors = sensorMgr.getSensorList(Sensor.TYPE_ALL);
        ...
        lsn = new SensorEventListener() {
         ...
         Log.i("zp","x = " + x + " y = " + y + " z = " + z);
          if ((z <= 6.0f) & (y <= 1.0f) & (x <= -7.0f)) {
							temp2[1] = true;
							imageGsensor.setImageResource(images_gsensor[1]);
						} else if ((z <= 7.0f) & (y <= 1.0f) & (x >= 6.0f)) {
							temp2[3] = true;
							imageGsensor.setImageResource(images_gsensor[3]);
						} else if ((x <= 1.0f) & (z <= 7.0f) & (y >= 7.0f)) {
							temp2[0] = true;
							imageGsensor.setImageResource(images_gsensor[0]);
						} else if ((x <= 1.0f) && (z <= 7.0f) && (y <= -7.0f)) {
							temp2[2] = true;
							imageGsensor.setImageResource(images_gsensor[2]);
						} else {
							imageGsensor.setImageResource(images_gsensor[2]);
						}

        if (temp2[0] && temp2[1] && temp2[3]) {
