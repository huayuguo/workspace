    ...
    public void testSetCurrentSubscriptions() {
        ...
        for (int i = 0; i < testSubscriptions.length; i++) {
            if (i != indexToSkipController) {
                // Force the test controllers into NetworkController.
                ///M:Avoid mobilesignalcontrollers.msubinfo is null.
                SubscriptionInfo mockSInfo = Mockito.mock(SubscriptionInfo.class);
                Mockito.when(mockSInfo.getSubscriptionId()).thenReturn(testSubscriptions[i]);
                Mockito.when(mockSInfo.getSimSlotIndex()).thenReturn(testSubscriptions[i]);
                mobileSignalControllers[i].mSubscriptionInfo = mockSInfo;
                /// @}
                mNetworkController.mMobileSignalControllers.put(testSubscriptions[i],
                        mobileSignalControllers[i]);
            }
