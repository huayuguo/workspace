...
public class SettingsActivity extends SettingsDrawerActivity
    ...
    private void doUpdateTilesList() {
        ...
        int hceFlg = android.provider.Settings.Global.getInt(
                getContentResolver(),
                com.mediatek.provider.MtkSettingsExt.Global.NFC_HCE_ON, 0);
        somethingChanged = setTileEnabled(new ComponentName(packageName,
                        Settings.PaymentSettingsActivity.class.getName()),
                pm.hasSystemFeature(PackageManager.FEATURE_NFC)
                && pm.hasSystemFeature(PackageManager.FEATURE_NFC_HOST_CARD_EMULATION)
                && adapter != null && adapter.isEnabled() && (hceFlg == 1), isAdmin)
                || somethingChanged;
        ...
        if (UserManager.isDeviceInDemoMode(this)) {
            List<DashboardCategory> categories = mDashboardFeatureProvider.getAllCategories();
            for (DashboardCategory category : categories) {
                final int tileCount = category.getTilesCount();
                for (int i = 0; i < tileCount; i++) {
                    ComponentName component = category.getTile(i).intent.getComponent();
                    if (ArrayUtils.contains(SettingsGateway.EXTRA_PKG_FOR_UNRESTRICTED,
                ...
                for (DashboardCategory category : categories) {
                    ...
                    for (int i = 0; i < tileCount; i++) {
                        ...
                        if (packageName.equals(component.getPackageName())
                            ...
                            somethingChanged = setTileEnabled(component, false, isAdmin)
                                    || somethingChanged;
                        /// M: When on restricted users, disable specified components @{
                        } else if (ArrayUtils.contains(SettingsGateway.EXTRA_PKG_FOR_UNRESTRICTED,
                                component.getPackageName())) {
                            somethingChanged = setTileEnabled(component, false, isAdmin)
                                    || somethingChanged;
                        /// M: @}
