...
public class AdvancedWifiCallingSettings extends SettingsPreferenceFragment
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ImsManager.ACTION_IMS_REGISTRATION_ERROR)) {
                ...
                showAlert(intent);
                    ...
                    Log.d(TAG, "config changed, finish WFC activity");
                    getActivity().finish();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "onCreate setWfcMode: Cellular Preferred");
    ...
    public void onResume() {
        ...
        if (telephonyManager.isNetworkRoaming()) {
             Log.d(TAG, "Phone is in Roaming");
             if (roamingMode == 1) {
                 ImsManager.setWfcMode(mContext,
                       ImsConfig.WfcModeFeatureValueConstants.CELLULAR_PREFERRED);
                 Log.d(TAG, "onResume setWfcMode: Cellular Preferred");
             } else if (roamingMode == 0) {
                 ImsManager.setWfcMode(mContext,
                         ImsConfig.WfcModeFeatureValueConstants.WIFI_PREFERRED);
                 Log.d(TAG, "onResume setWfcMode: WiFi Preferred");
             }
        }
    ...
    public void onSwitchChanged(final Switch switchView, boolean isChecked) {
        ...
        if (isChecked) {
            ...
            ImsManager.setWfcSetting(mContext, isChecked);
            ...
            if (sSwitchFlag) {
                ...
                mSwitchBar.setTextViewLabel(true);
                AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
                builder.setCancelable(false);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mButtonRoaming) {
            String roamingMode = (String) newValue;
            mButtonRoaming.setValue(roamingMode);
            Log.d(TAG, "onPreferenceChange , newValue = " + newValue);
            Settings.Global.putInt(getContentResolver(), "ROAMING_MODE",
                    Integer.parseInt(roamingMode));
        }
        return true;
    }
