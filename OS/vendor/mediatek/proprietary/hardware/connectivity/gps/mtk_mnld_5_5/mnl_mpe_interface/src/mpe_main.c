        ...
        case CMD_GET_ADR_STATUS_REQ: {
            if(mpe_sensor_get_listen_mode() == MPE_START_MODE) {
                ...
                if(mnl_out.valid_flag[MTK_MPE_KER_FLAG] && mnl_out.valid_flag[MTK_MPE_RAW_FLAG] /*&& accuracy[SENSOR_TYPE_GYR] >= 2*/) {
                    LOGD("mnl_out, %lf, %lf, %f, %f, %d, %f, %f, %f, %f\n",
                        mnl_out.latitude, mnl_out.longitude, mnl_out.velocity[0], mnl_out.bearing, mnl_out.staticIndex, mnl_out.barometerHeight, mnl_out.HACC, gyr_temperature, gyr_scp_calib_done);
    ...
    for(i = 0; i < len; i++ ) {
        if(data + i != NULL) {
            if (mpe_sys_get_mpe_conf_flag() & MPE_CONF_PRT_RAWDATA) {
                LOGD("[%d] MPErpy %lld %f %f %f %f %f %f %f %f %f %f %lf %lf %lf %lf %lf %lf %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %u %u %f",
