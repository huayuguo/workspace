...
******************************************************************************/
#if 0
#define GPS_DBG_FUNC(fmt, arg...)    pr_info(PFX, "%s: " fmt, __func__, ##arg)
...
#else
#define PFX "GPS: "
#define GPS_DBG_NONE(fmt, arg...)    do {pr_info(PFX "%s: " fmt, __func__, ##arg);} while (0)
...
static int mt3333_power_on(int state);
static int mt3333_power_off(int state);

struct mt3326_gps_hardware mt3326_gps_hw = {
	.ext_power_on =  mt3333_power_on,
	.ext_power_off = mt3333_power_off,
};
/******************************************************************************
 * local variables
******************************************************************************/
...
static struct pinctrl_state *mt3333_lna_init;
static struct pinctrl_state *mt3333_lna_oh;
static struct pinctrl_state *mt3333_lna_ol;
static struct pinctrl *mt3333_pinctrl;

static int mt3333_get_pinctrl(struct platform_device *pdev)
{
	GPS_DBG("ENTER++\n");

	mt3333_pinctrl = devm_pinctrl_get(&pdev->dev);
	if(IS_ERR(mt3333_pinctrl)) {
		GPS_ERR("cannot find mt3333 pinctrl\n");
		return -1;
	}
	mt3333_lna_init = pinctrl_lookup_state(mt3333_pinctrl, "mt3333_lna_state_init");
	if(mt3333_lna_init == NULL) {
		GPS_ERR("cannot find mt3333 lna pin init state\n");
		return -2;
	}
	//pinctrl_select_state(mt3333_pinctrl, mt3333_lna_init);		
	mt3333_lna_oh = pinctrl_lookup_state(mt3333_pinctrl, "mt3333_lna_state_oh");
	if(mt3333_lna_oh == NULL) {
		GPS_ERR("cannot find mt3333 lna pin oh state\n");
		return -3;
	}
	mt3333_lna_ol = pinctrl_lookup_state(mt3333_pinctrl, "mt3333_lna_state_ol");
	if(mt3333_lna_ol == NULL) {
		GPS_ERR("cannot find mt3333 lna pin ol state\n");
		return -4;
	}
...
{
	if(mt3333_pinctrl) {
		pinctrl_select_state(mt3333_pinctrl, mt3333_lna_oh);		
		GPS_DBG("set gps lna to oh\n");
		return 0;
	} else 
		return 1;
}
...
{
	if(mt3333_pinctrl) {
		pinctrl_select_state(mt3333_pinctrl, mt3333_lna_ol);		
		GPS_DBG("set gps lna to ol\n");
		return 0;
	} else 
		return 1;
}
